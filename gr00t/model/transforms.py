...
def _ensure_transformers_eagle_processor_compat() -> None:
    """Keep remote Eagle processor code compatible with nearby transformers releases."""
    try:
        from collections.abc import Mapping

        from transformers.processing_utils import ProcessorMixin as TransformersProcessorMixin

        if not getattr(TransformersProcessorMixin, "_awesome_qvla_validate_init_compat", False):
            original_validate_init_kwargs = TransformersProcessorMixin.validate_init_kwargs

            def _validate_init_kwargs_compat(processor_config, valid_kwargs):
                unused_kwargs = original_validate_init_kwargs(processor_config, valid_kwargs)
                if isinstance(unused_kwargs, Mapping):
                    return dict(unused_kwargs)
                if isinstance(unused_kwargs, (set, list, tuple)):
                    normalized = {}
                    for item in unused_kwargs:
                        if isinstance(item, str):
                            if item in processor_config:
                                normalized[item] = processor_config[item]
                            continue
                        try:
                            key, value = item
                        except Exception:
            ...
            TransformersProcessorMixin.validate_init_kwargs = staticmethod(
                _validate_init_kwargs_compat
            )
            TransformersProcessorMixin._awesome_qvla_validate_init_compat = True
    except Exception:
        pass
    ...
    try:
        ...
        original_group_images_by_shape = image_transforms.group_images_by_shape

        if not getattr(original_group_images_by_shape, "_awesome_qvla_compat", False):

            def _group_images_by_shape_compat(images, disable_grouping=False, *args, **kwargs):
                kwargs.setdefault("disable_grouping", disable_grouping)
                try:
                    return original_group_images_by_shape(images, *args, **kwargs)
                except TypeError as exc:
                    if "disable_grouping" not in str(exc):
                        raise
                    kwargs.pop("disable_grouping", None)
                    return original_group_images_by_shape(images, *args, **kwargs)

            _group_images_by_shape_compat._awesome_qvla_compat = True
            image_transforms.group_images_by_shape = _group_images_by_shape_compat
            fast_utils.group_images_by_shape = _group_images_by_shape_compat
        else:
            fast_utils.group_images_by_shape = original_group_images_by_shape

        try:
            from transformers.image_processing_utils import BASE_IMAGE_PROCESSOR_DOCSTRING
        except Exception:
            BASE_IMAGE_PROCESSOR_DOCSTRING = "Base fast image processor."

        fast_docstring_aliases = {
            "BASE_IMAGE_PROCESSOR_FAST_DOCSTRING": BASE_IMAGE_PROCESSOR_DOCSTRING,
            "BASE_IMAGE_PROCESSOR_FAST_DOCSTRING_PREPROCESS": BASE_IMAGE_PROCESSOR_DOCSTRING,
        }
        for name, value in fast_docstring_aliases.items():
            if not hasattr(fast_utils, name):
                setattr(fast_utils, name, value)

        previous_getattr = getattr(fast_utils, "__getattr__", None)

        def _compat_fast_utils_getattr(name: str) -> str:
            if name.startswith("BASE_IMAGE_PROCESSOR_FAST_DOCSTRING"):
                setattr(fast_utils, name, BASE_IMAGE_PROCESSOR_DOCSTRING)
                return BASE_IMAGE_PROCESSOR_DOCSTRING
            if previous_getattr is not None:
                return previous_getattr(name)
            raise AttributeError(f"module {fast_utils.__name__!r} has no attribute {name!r}")

        fast_utils.__getattr__ = _compat_fast_utils_getattr
    except Exception:
        pass


def _patch_eagle_image_processor_kwargs(eagle_processor: ProcessorMixin) -> None:
    image_processor = getattr(eagle_processor, "image_processor", None)
    if image_processor is None or getattr(image_processor, "_awesome_qvla_preprocess_compat", False):
        return

    preprocess = getattr(image_processor, "_preprocess", None)
    if preprocess is None:
        return

    def _preprocess_compat(*args, **kwargs):
        kwargs.pop("disable_grouping", None)
        return preprocess(*args, **kwargs)

    image_processor._preprocess = _preprocess_compat
    image_processor._awesome_qvla_preprocess_compat = True
...
def build_eagle_processor(eagle_path: str) -> ProcessorMixin:
    _ensure_transformers_eagle_processor_compat()
    eagle_processor = AutoProcessor.from_pretrained(
        eagle_path, trust_remote_code=True, use_fast=True
    )
    _patch_eagle_image_processor_kwargs(eagle_processor)
